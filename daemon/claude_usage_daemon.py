...
ESP32 "DualMeter" peripheral over a custom GATT service. Uses
...
from codex_limits import poll_codex_payload

DEVICE_NAME = "DualMeter"
...
def _extract_access_token(blob: str) -> str | None:
    ...
    if isinstance(data, dict):
        # Prefer the Claude Code OAuth blob. Keychain often also stores MCP
        # plugin tokens under mcpOAuth, and iterating values() would pick one
        # of those first — which then 401s against api.anthropic.com.
        ...
        # nested fallback, skip MCP plugin tokens
    ...
    try:
        while client.is_connected and not stop_event.is_set():
            ...
            if session.refresh_requested.is_set() or elapsed >= POLL_INTERVAL:
                ...
                try:
                    codex = await poll_codex_payload()
                except Exception as e:
                    log(f"Codex poll failed: {e}")
                    codex = None
                if payload is None:
                    payload = {"ok": False, "s": 0, "sr": -1, "w": 0, "wr": -1, "st": "unknown"}
                if codex:
                    payload.update(codex)
                else:
                    payload["xok"] = False
                if payload.get("ok") or payload.get("xok"):
                    if await session.write_payload(payload):
                        last_poll = time.time()
                        used_successfully = True
                elif dead:
                    log("No usable Claude or Codex token; signalling no-data")
                    if await session.write_payload({"ok": False, "xok": False}):
                        last_poll = time.time()
                else:
