...
static bool parse_json(const char* json, UsageData* out) {
    ...
    out->codex_valid = !doc["xs"].isNull() || !doc["xw"].isNull();
    out->codex_session_pct = doc["xs"].isNull() ? -1.0f : (doc["xs"] | 0.0f);
    out->codex_session_reset_mins = doc["xsr"] | -1;
    out->codex_weekly_pct = doc["xw"].isNull() ? -1.0f : (doc["xw"] | 0.0f);
    out->codex_weekly_reset_mins = doc["xwr"] | -1;
    out->codex_ok = doc["xok"] | out->codex_valid;
    return true;
}
    ...
    while (Serial.available()) {
        ...
        if (c == '\n' || c == '\r') {
            ...
            if (strcmp(cmd_buf, "screenshot") == 0) send_screenshot();
                ...
                ui_update(&usage);
                Serial.println("demo payload applied");
            } else if (cmd_buf[0] == '{') {
